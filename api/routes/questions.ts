...
router.post('/batch', authMiddleware, async (req, res) => {
  try {
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
      return res.status(400).json({ success: false, error: '请提供题目数据' });
    }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      try {
        ...
        errors.push(`第${q.row || (i + 2)}行: ${(error as Error).message}`);
      }
    }
    ...
    res.status(500).json({ success: false, error: '批量创建题目失败' });
  }
});
