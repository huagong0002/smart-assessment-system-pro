...
router.post('/', authMiddleware, roleMiddleware(['admin']), async (req: AuthRequest, res) => {
  try {
    ...
    console.log('[Class Create] Request body:', req.body);
    console.log('[Class Create] name:', name, 'teacher_id:', teacher_id, 'grade:', grade);

    // 验证 teacher_id 是否存在于 teachers 表中
    let teacherId = null;
    if (teacher_id && teacher_id !== '' && teacher_id !== 'undefined') {
      const teacher = await db.get('SELECT id FROM teachers WHERE id = ?', [parseInt(teacher_id)]);
      if (teacher) {
        teacherId = teacher.id;
      } else {
        console.warn('[Class Create] Teacher not found, setting teacher_id to null:', teacher_id);
      }
    }

    const result = await db.run(
      'INSERT INTO classes (name, teacher_id, grade) VALUES (?, ?, ?)',
      [name, teacherId, parseInt(grade)]
    );
    ...
    console.error('[Class Create] Error stack:', error.stack);
    res.status(500).json({ message: '创建班级失败: ' + (error.message || '未知错误') });
  }
});
