...
router.put('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const db = await getDb();
    const examId = req.params.id;
    const { name, course_type, grade, time_limit } = req.body;
    ...
    await db.run(
      `UPDATE exams SET
        ...
        updated_at = datetime('now')
      WHERE id = ?`,
      [name, course_type, grade, time_limit, examId]
    );

    res.json({ success: true, message: '更新成功' });
  } catch (error: any) {
    console.error('[Update Exam] Error:', error);
    res.status(500).json({ message: '更新失败: ' + (error.message || '未知错误') });
  }
});
...
router.delete('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // 检查是否有测评记录
    const recordCount = await db.get('SELECT COUNT(*) as count FROM exam_records WHERE exam_id = ?', [examId]);
    if (recordCount?.count > 0) {
      res.status(400).json({ message: '该试卷已有测评记录，无法删除' });
      return;
    }
    ...
    res.status(500).json({ message: '删除失败: ' + (error.message || '未知错误') });
  }
});
