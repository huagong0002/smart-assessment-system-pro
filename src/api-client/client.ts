...
export async function apiFetch(path: string, options: RequestInit = {}) {
  ...
  // 处理空响应体（如 DELETE 请求�?  const text = await response.text();
  if (!text) {
    return { message: '操作成功' };
  }
...
export const examApi = {
  ...
  update: (id: number, data: { name: string; course_type: string; grade: number; time_limit: number }) =>
    apiFetch(`/exams/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  delete: (id: number) => apiFetch(`/exams/${id}`, { method: 'DELETE' }),
...
export const questionApi = {
  ...
  bulkCreate: (questions: any[]) => apiFetch('/questions/batch', { method: 'POST', body: JSON.stringify(questions) }),
};
